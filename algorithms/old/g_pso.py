...
# ----------------------------
# Helper: Simulate task execution time based on "MIPS"
# ----------------------------

def estimate_task_time(task_mi: float, vm_mips: float) -> float:
    """ETC(i,j) = MI_i / MIPS_j  (from paper Eq.1)"""
...
def compute_makespan(schedule: List[int], tasks_mi: List[float], vm_mips_list: List[float]) -> float:
    ...
    for task_id, vm_idx in enumerate(schedule):
        exec_time = estimate_task_time(tasks_mi[task_id], vm_mips_list[vm_idx])
...
# ----------------------------
# Main G&PSO Scheduler
# ----------------------------

def run_g_pso(tasks: List[float], vm_names: List[str], logger=None) -> List[Dict]:
    """
    G&PSO Algorithm (based on Zhong et al., 2016)
    - tasks: list of task sizes in "simulated seconds" → we treat them as MI
    - vm_names: list of VM names (e.g., ["vm1", "vm2", ..., "vm5"])
    - Note: tasks durations are interpreted as MI; VM CPU limits from .env → MIPS
    """
    if logger:
    ...
    # === Map VM CPU limits to MIPS (scale 0.5–1.0 → 500–1000 MIPS as in paper) ===
    # Assume: CPU limit 0.5 → 500 MIPS, 1.0 → 1000 MIPS (linear)
    from dotenv import load_dotenv
    import os
    load_dotenv()
    vm_mips = []
    for i, vm in enumerate(vm_names):
        cpu_quota = float(os.getenv(f"VM{i+1}_CPUS", "0.5"))
        mips = 500 + (cpu_quota - 0.5) * (1000 - 500) / (1.0 - 0.5)  # linear mapping
        vm_mips.append(mips)
    tasks_mi = tasks  # treat input task durations as MI values
    ...
    for _ in range(num_particles):
        ...
        fit = 1.0 / compute_makespan(pos, tasks_mi, vm_mips)
        pbest_fit.append(fit)

    # === Greedy Initialization for gbest ===
    gov = greedy_initialization(tasks_mi, vm_mips)
    gbest = gov.copy()
    gct = compute_makespan(gov, tasks_mi, vm_mips)
    gbest_fit = 1.0 / gct
    ...
    for it in range(max_iter):
        for i in range(num_particles):
            ...
            # Evaluate fitness
            makespan = compute_makespan(particles[i], tasks_mi, vm_mips)
    ...
    for i, vm in enumerate(final_schedule_vm_names):
        msg = f"G&PSO: Task {i} ({tasks[i]:.2f}s) → {vm}"
        if logger:
            logger.log(msg)
        else:
            print(msg)
        start, end = run_task_in_vm(vm, tasks[i])
        task_events.append({
            "task_id": i,
            "duration": tasks[i],
            "vm": vm,
